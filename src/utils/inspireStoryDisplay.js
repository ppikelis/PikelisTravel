(function (global) {
  ...
  function formatInspireDurationDays(value) {
    try {
  ...
  function formatInspireDifficultyLabel(raw) {
    try {
  ...
  function formatInspireJourneyCategoryLabel(raw) {
    try {
      if (raw === null || raw === undefined) return "";
      if (typeof raw === "object") return "";
      const s = typeof raw === "number" && Number.isFinite(raw) ? String(raw) : String(raw);
  ...
  function getInspireStoryGeoFields(metadata) {
    ...
    const continentRaw =
      pickFirstString(geo, ["continent", "region_continent"]) ||
      pickFirstString(m, ["continent", "region_continent"]);
    const continent = formatInspireJourneyCategoryLabel(humanizeGeoToken(continentRaw) || continentRaw);
    return { country, continent };
  }

  function getInspireStoryGeoLabel(metadata) {
    try {
      const m = safeMetadata(metadata);
  ...
  function durationDaysNumber(m) {
    try {
      ...
      const raw =
        timing.duration_days != null ? timing.duration_days :
        m.duration_days != null ? m.duration_days :
        m.trip_duration_days != null ? m.trip_duration_days :
        m.journey_duration_days != null ? m.journey_duration_days :
  ...
  function getInspireStoryCategoryDurationLine(metadata) {
    try {
      const m = safeMetadata(metadata);
  ...
  function getInspireStoryDifficulty(metadata) {
    try {
      const m = safeMetadata(metadata);
  ...
  function getInspireStoryGuideUrl(metadata) {
    try {
      const m = safeMetadata(metadata);
      const g = m.guide;
  ...
  function inspireStoryHasGuide(metadata) {
    try {
      const m = safeMetadata(metadata);
  ...
  function getInspireStoryHeroAlt(story) {
    try {
  ...
  function getInspireStoryMarkdownExcerpt(storyOrMarkdown, options) {
    var opts = options || {};
    let raw = "";
    try {
      const max = Math.min(Math.max(Number(opts.maxLength) || 160, 20), 200000);
  ...
  function getInspireFeaturedCardDisplay(story) {
    const fallback = {
      geoLabel: "",
  ...
  function normalizeInspireSearchQuery(query) {
    try {
  ...
  function inspireMetadataActivityTagsText(m) {
    try {
      const parts = [];
      const cls = inspireClassificationObject(m);
      const clsTags = cls.activity_tags != null ? cls.activity_tags : cls.tags;
  ...
  function getInspireStorySearchHaystack(story) {
    try {
  ...
  function matchesInspireStorySearch(story, query) {
    try {
  ...
  function getInspireStorySortDateMillis(story) {
    try {
  ...
  function getInspireStoryDifficultySortRank(story) {
    try {
      const m = safeMetadata(story && story.metadata);
  ...
  function cmpDifficulty(a, b) {
    ...
    const ma = safeMetadata(a && a.metadata);
    const mb = safeMetadata(b && b.metadata);
  ...
  function sortInspireStoriesByKey(stories, sortKey) {
    try {
  ...
  function inspireListPrimaryDestinationIsGuide(metadata) {
    try {
      const m = safeMetadata(metadata);
  ...
  function getInspireStoryListCardHref(story) {
    try {
      if (!story || typeof story !== "object") return "";
      const m = safeMetadata(story.metadata);
  ...
  const INSPIRE_FACET_UNSPECIFIED = "(unspecified)";
  ...
  function projectInspireStoryFacetValues(story) {
    ...
    try {
      ...
      const suitableFor = uniqueSortedFacetValues(suitLabels.concat(suitableFromFlat));
      const suitableForOut = suitableFor.length ? suitableFor : [U];
      const timing = inspireTimingObject(m);
      const bestSeasons = timing.best_seasons != null ? timing.best_seasons : (timing.seasons != null ? timing.seasons : m.best_seasons);
      const seasonFromTiming = Array.isArray(bestSeasons)
        ? bestSeasons.filter((x) => typeof x === "string" && x.trim()).map((x) => humanizeGeoToken(x))
        : [];
      const seasonVals = uniqueSortedFacetValues(seasonFromTiming.concat(flattenMetaMulti(m, ["season", "travel_season", "best_season", "months", "ideal_months"])));
      ...
      const budgetVals = uniqueSortedFacetValues(
        (budgetStr ? [humanizeGeoToken(budgetStr.replace(/_/g, " ")) || budgetStr] : []).concat(flattenMetaMulti(m, ["budget", "budget_level", "cost_level", "trip_budget"]))
      );
  ...
  function buildInspireFacetOptions(stories) {
    const dims = ["continent", "country", "duration", "activity", "difficulty", "suitableFor", "season", "budget", "guide"];
    const acc = { continent: new Set(), country: new Set(), duration: new Set(), activity: new Set(), difficulty: new Set(), suitableFor: new Set(), season: new Set(), budget: new Set(), guide: new Set() };
  ...
  function createEmptyInspireFacetSelection() {
    return { continent: [], country: [], duration: [], activity: [], difficulty: [], suitableFor: [], season: [], budget: [], guide: [] };
  }

  function inspireFacetSelectionHasActive(selection) {
    try {
  ...
  function storyPassesInspireFacetSelection(story, selection) {
    try {
